...
namespace Open_XML_Project
{
    class Program
    {
        static void Main(string[] args)
        {
            string filepath = @"C:\Users\EmileMesselken\Desktop\OpenXML_PP.pptx";
            string imagepath = @"C:\Users\EmileMesselken\Desktop\Powerpoint-Project\Insert_pictures_here";

            CreatePresentation(filepath);

            string[] filePaths = Directory.GetFiles(imagepath, "*.jpg");

            AddImage(filepath, filePaths[0]);

            for (int i = 1; i < filePaths.Length; i++)
            {
                u = i;
                InsertNewSlide(filepath, i + 1, "");
                AddImage(filepath, filePaths[i]);
            }
        }
